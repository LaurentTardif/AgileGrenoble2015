...
public class DelimitedStreamReader {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DelimitedStreamReader.class);
    ...
    private String readLine(boolean trim) throws IOException {
        ...
        while (!done) {
            ...
            LOG.info("Going to read a line (offset): "+offset);
            ...
            LOG.info("Going to read a line :(end) "+end);
            if (!done) {
        ...
        LOG.info("Going to read a line(length) : "+strLength);
        strBufferIndex = 0;
